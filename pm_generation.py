...
def data_preparation(args):
    ...
    for set_info in dataset:
        ...
        src[set_info], tgt[set_info] = generate_tmp_file(os.path.join(args.data_dir, "{}.pm".format(set_info)), wiki_kb[set_info], verbose=True)
...
def train(args):
    ...
    script = "python {}/train.py -data {} -save_model {} -gpuid 0 \
           -global_attention {} -batch_size {} -encoder_type {} -rnn_type {}".format(OpenNMT_dir, args.data, args.model, attention, batch_size, encoder_type, rnn_type)
    print(script)
    os.system(script)
